  ...
  asyncHandler(async (req, res) => {
    ...
    if (token && excludeSelf === "true") {
      try {
        ...
        console.warn(
          "Invalid token provided, proceeding without exclusion:",
          err.message
        );
      }
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.files) {
      if (req.files.bannerImage) {
        ...
        storeData.bannerImage = imageURL; // Filename only
      }
      if (req.files.featuredImage) {
        ...
        storeData.featuredImage = imageURL; // Filename only
      }
      if (req.files.logo) {
        ...
        storeData.logo = imageURL; // Filename only
      }
    }
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.files) {
      if (req.files.bannerImage) {
        const fileName = `${Date.now()}-${req.files.bannerImage.name.replace(
          /[^a-zA-Z0-9.-]/g,
          "_"
        )}`;
      ...
      if (req.files.featuredImage) {
        const fileName = `${Date.now()}-${req.files.featuredImage.name.replace(
          /[^a-zA-Z0-9.-]/g,
          "_"
        )}`;
        const filePath = `/Uploads/${fileName}`;
      ...
      if (req.files.logo) {
        const fileName = `${Date.now()}-${req.files.logo.name.replace(
          /[^a-zA-Z0-9.-]/g,
          "_"
        )}`;
        const filePath = `/Uploads/${fileName}`;
