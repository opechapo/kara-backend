...
const getNonce = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (error.code === 11000) {
      console.warn(
        "Duplicate key error on walletAddress, fetching existing user:",
        normalizedAddress
      );
      try {
        ...
        if (existingUser) {
          ...
          return res.json({
            success: true,
            data: { nonce: existingUser.nonce || nonce },
          });
        }
      } catch (retryError) {
      ...
      res.status(500);
      throw new Error(
        "Failed to generate nonce: Unable to resolve duplicate wallet address"
      );
    }
    res.status(500);
...
const connectWallet = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      res.status(401);
      throw new Error(
        `Invalid SIWE message or signature: ${
